...
def test_mqa(dim, dim_out, query_strides, kv_stride, use_bias, attn_mask):
    ...
    mod_mlx.query.layers[-1].weight = mx.array(
        mod_timm.query[-1].weight.detach().numpy()
    ).transpose(0, 2, 3, 1)

    mod_mlx.key.layers[-1].weight = mx.array(
        mod_timm.key[-1].weight.detach().numpy()
    ).transpose(0, 2, 3, 1)

    mod_mlx.value.layers[-1].weight = mx.array(
        mod_timm.value[-1].weight.detach().numpy()
    ).transpose(0, 2, 3, 1)

    mod_mlx.output.layers[-2].weight = mx.array(
        mod_timm.output[-2].weight.detach().numpy()
    ).transpose(0, 2, 3, 1)

    if use_bias:
        mod_mlx.query.layers[-1].bias = mx.array(
            mod_timm.query[-1].bias.detach().numpy()
        )

        mod_mlx.key.layers[-1].bias = mx.array(
            mod_timm.key[-1].bias.detach().numpy(),
        )

        mod_mlx.value.layers[-1].bias = mx.array(
            mod_timm.value[-1].bias.detach().numpy()
        )

        mod_mlx.output.layers[-2].bias = mx.array(
            mod_timm.output[-2].bias.detach().numpy()
        )

    if kv_stride > 1:
        mod_mlx.key.layers[0].weight = mx.array(
            mod_timm.key[0].weight.detach().numpy()
        ).transpose(0, 2, 3, 1)

        mod_mlx.value.layers[0].weight = mx.array(
            mod_timm.value[0].weight.detach().numpy()
        ).transpose(0, 2, 3, 1)
