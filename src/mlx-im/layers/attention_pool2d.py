...
class RotAttentionPool2d(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = RotaryEmbedding(
            self.head_dim, in_pixels=False, ref_feat_shape=ref_feat_size
        )
    ...
    def forward(self, x, pre_logits: bool = False):
        ...
        q = torch.cat(
            [q[:, :, :1, :], apply_rot_embed(q[:, :, 1:, :], rse, rce)], dim=2
        ).type_as(v)
        k = torch.cat(
            [k[:, :, :1, :], apply_rot_embed(k[:, :, 1:, :], rse, rce)], dim=2
        ).type_as(v)
...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x, pre_logits: bool = False):
        ...
        pos_embed = resample_abs_pos_embed(
            self.pos_embed.unsqueeze(0), (H, W), num_prefix_tokens=1
        )
        x = x + pos_embed
