...
class FocalNetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ls1 = (
            LayerScale2d(dim, layerscale_value)
...
def checkpoint_filter_fn(state_dict, model: FocalNet):
    ...
    for k, v in state_dict.items():
        ...
        if (
            k in dest_dict
            and dest_dict[k].numel() == v.numel()
            and dest_dict[k].shape != v.shape
        ):
...
def focalnet_xlarge_fl3(pretrained=False, **kwargs) -> FocalNet:
    ...
    return _create_focalnet(
        "focalnet_xlarge_fl3", pretrained=pretrained, **model_kwargs
    )
...
def focalnet_xlarge_fl4(pretrained=False, **kwargs) -> FocalNet:
    ...
    return _create_focalnet(
        "focalnet_xlarge_fl4", pretrained=pretrained, **model_kwargs
    )
