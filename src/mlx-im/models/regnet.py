...
def adjust_widths_groups_comp(widths, bottle_ratios, groups, min_ratio=0.0):
    ...
    if min_ratio:
        ...
        bottleneck_widths = [
            make_divisible(w_bot, g, min_ratio)
...
def generate_regnet(width_slope, width_initial, width_mult, depth, group_size, quant=8):
    ...
    widths = (
        np.round(np.divide(width_initial * np.power(width_mult, width_exps), quant))
        * quant
    )
    ...
    if "classy_state_dict" in state_dict:
        ...
        for k, v in state_dict["trunk"].items():
            ...
            k = re.sub(
                ...
                lambda x: f"s{int(x.group(1))}.b{int(x.group(2)) + 1}",
                k,
            )
    ...
    if "stem.0.weight" in state_dict:
        ...
        for k, v in state_dict.items():
            ...
            k = re.sub(
                ...
                lambda x: f"s{int(x.group(1))}.b{int(x.group(3)) + 1}",
                k,
            )
            for s, r in replaces:
                k = k.replace(s, r)
...
model_cfgs = dict(
    ...
    regnetx_004_tv=RegNetCfg(
        w0=24, wa=24.48, wm=2.54, group_size=16, depth=22, group_min_ratio=0.9
    ),
    ...
    regnety_640=RegNetCfg(
        w0=352, wa=147.48, wm=2.4, group_size=328, depth=20, se_ratio=0.25
    ),
    regnety_1280=RegNetCfg(
        w0=456, wa=160.83, wm=2.52, group_size=264, depth=27, se_ratio=0.25
    ),
    regnety_2560=RegNetCfg(
        w0=640, wa=230.83, wm=2.53, group_size=373, depth=27, se_ratio=0.25
    ),
