...
class HieraBlock(nn.Module):
    def __init__(
        ...
        self.ls1 = (
            LayerScale(dim_out, init_values=init_values)
        ...
        if sep_pos_embed:
            ...
            if abs_win_pos_embed:
                ...
                self.pos_embed = nn.Parameter(
                    torch.zeros(1, embed_dim, *global_pos_size)
                )
                self.pos_embed_win = nn.Parameter(
                    torch.zeros(1, embed_dim, *mask_unit_size)
                )
            else:
        ...
        for i, blk in enumerate(blocks):
            ...
            if i in take_indices:
                ...
                intermediates.append(
                    x_int.permute(0, 3, 1, 2) if output_fmt == "NCHW" else x_int
                )
...
def hiera_small_abswin_256(pretrained=False, **kwargs):
    ...
    return _create_hiera(
        "hiera_small_abswin_256", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def hiera_base_abswin_256(pretrained=False, **kwargs):
    ...
    return _create_hiera(
        "hiera_base_abswin_256", pretrained=pretrained, **dict(model_args, **kwargs)
    )
