...
class ConvNorm(nn.Module):
    def __init__(
        ...
        self.linear = nn.Conv2d(
            in_chs, out_chs, kernel_size, stride, padding, dilation, groups, bias=False
        )
...
class Stem8(nn.Sequential):
    def __init__(self, in_chs, out_chs, act_layer):
        ...
        self.add_module(
            "conv2", ConvNorm(out_chs // 4, out_chs // 2, 3, stride=2, padding=1)
        )
        self.add_module("act2", act_layer())
        self.add_module(
            "conv3", ConvNorm(out_chs // 2, out_chs, 3, stride=2, padding=1)
        )
...
class Stem16(nn.Sequential):
    def __init__(self, in_chs, out_chs, act_layer):
        ...
        self.add_module("act1", act_layer())
        self.add_module(
            "conv2", ConvNorm(out_chs // 8, out_chs // 4, 3, stride=2, padding=1)
        )
        self.add_module("act2", act_layer())
        self.add_module(
            "conv3", ConvNorm(out_chs // 4, out_chs // 2, 3, stride=2, padding=1)
        )
        self.add_module("act3", act_layer())
        self.add_module(
            "conv4", ConvNorm(out_chs // 2, out_chs, 3, stride=2, padding=1)
        )
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        pos = torch.stack(
            ndgrid(torch.arange(resolution[0]), torch.arange(resolution[1]))
        ).flatten(1)
...
class AttentionDownsample(nn.Module):
    ...
    def __init__(
        ...
        if self.use_conv:
            ...
            sub_layer = partial(
                nn.AvgPool2d,
                kernel_size=3 if use_pool else 1,
                padding=1 if use_pool else 0,
        ...
        self.proj = nn.Sequential(
            OrderedDict(
                [("act", act_layer()), ("ln", ln_layer(self.val_attn_dim, out_dim))]
            )
        )
...
class LevitDownsample(nn.Module):
    def __init__(
        ...
        self.mlp = LevitMlp(
            out_dim, int(out_dim * mlp_ratio), use_conv=use_conv, act_layer=act_layer
        )
...
class LevitBlock(nn.Module):
    def __init__(
        ...
        self.mlp = LevitMlp(
            dim, int(dim * mlp_ratio), use_conv=use_conv, act_layer=act_layer
        )
...
class Levit(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_stages):
            ...
            self.feature_info += [
                dict(num_chs=embed_dim[i], reduction=stride, module=f"stages.{i}")
            ]
            in_dim = embed_dim[i]
        ...
        # Classifier head
        self.head = (
            NormLinear(embed_dim[-1], num_classes, drop=drop_rate)
            if num_classes > 0
            else nn.Identity()
        )
...
def levit_512_s8(pretrained=False, **kwargs) -> Levit:
    return create_levit(
        "levit_512_s8", pretrained=pretrained, distilled=False, **kwargs
    )
...
def levit_conv_128s(pretrained=False, **kwargs) -> Levit:
    return create_levit(
        "levit_conv_128s", pretrained=pretrained, use_conv=True, **kwargs
    )
...
def levit_conv_128(pretrained=False, **kwargs) -> Levit:
    return create_levit(
        "levit_conv_128", pretrained=pretrained, use_conv=True, **kwargs
    )
...
def levit_conv_192(pretrained=False, **kwargs) -> Levit:
    return create_levit(
        "levit_conv_192", pretrained=pretrained, use_conv=True, **kwargs
    )
...
def levit_conv_256(pretrained=False, **kwargs) -> Levit:
    return create_levit(
        "levit_conv_256", pretrained=pretrained, use_conv=True, **kwargs
    )
...
def levit_conv_384(pretrained=False, **kwargs) -> Levit:
    return create_levit(
        "levit_conv_384", pretrained=pretrained, use_conv=True, **kwargs
    )
...
def levit_conv_384_s8(pretrained=False, **kwargs) -> Levit:
    return create_levit(
        "levit_conv_384_s8", pretrained=pretrained, use_conv=True, **kwargs
    )
