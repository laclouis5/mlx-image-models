...
def load_state_dict_from_hf(
    ...
    _logger.debug(
        f"[{model_id}] Safe alternative not found for '{filename}'. Loading weights using default pytorch."
    )
    try:
        state_dict = torch.load(
            cached_file, map_location="cpu", weights_only=weights_only
        )
    except TypeError:
...
def save_config_for_hf(
    ...
    pretrained_cfg = filter_pretrained_cfg(
        model.pretrained_cfg, remove_source=True, remove_null=True
    )
