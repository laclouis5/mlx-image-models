...
class MobileNetV3(nn.Module):
    ...
    def __init__(
        ...
        self.num_features = (
            builder.in_chs
        )  # features of last stage, output of forward_features()
        ...
        if head_norm:
            ...
            self.conv_head = create_conv2d(
                num_pooled_chs, self.head_hidden_size, 1, padding=pad_type
            )  # never bias
        ...
        self.classifier = (
            Linear(self.head_hidden_size, num_classes)
            if num_classes > 0
            else nn.Identity()
        )
    ...
    **kwargs,
    ...
    model_kwargs = dict(
        block_args=decode_arch_def(
            arch_def, depth_multiplier=depth_multiplier, group_size=group_size
        ),
...
def mobilenetv4_conv_small_035(pretrained: bool = False, **kwargs) -> MobileNetV3:
    ...
    model = _gen_mobilenet_v4(
        "mobilenetv4_conv_small_035", 0.35, pretrained=pretrained, **kwargs
    )
    return model
...
def mobilenetv4_conv_small_050(pretrained: bool = False, **kwargs) -> MobileNetV3:
    """MobileNet V4"""
    model = _gen_mobilenet_v4(
        "mobilenetv4_conv_small_050", 0.50, pretrained=pretrained, **kwargs
    )
    return model
...
def mobilenetv4_hybrid_medium_075(pretrained: bool = False, **kwargs) -> MobileNetV3:
    """MobileNet V4 Hybrid"""
    model = _gen_mobilenet_v4(
        "mobilenetv4_hybrid_medium_075", 0.75, pretrained=pretrained, **kwargs
    )
    return model
...
def mobilenetv4_hybrid_large_075(pretrained: bool = False, **kwargs) -> MobileNetV3:
    """MobileNet V4 Hybrid"""
    model = _gen_mobilenet_v4(
        "mobilenetv4_hybrid_large_075", 0.75, pretrained=pretrained, **kwargs
    )
    return model
