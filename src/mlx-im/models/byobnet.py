...
class ByoBlockCfg:
    ...
    gs: Optional[Union[int, Callable]] = (
...
class ByoModelCfg:
    ...
    # Head config
    head_hidden_size: Optional[int] = (
        None  # feat dim of MLP head or AttentionPool output
    )
...
def _mobileone_bcfg(
    d=(2, 8, 10, 1), wf=(1.0, 1.0, 1.0, 1.0), se_blocks=(), num_conv_branches=1
):
    c = (64, 128, 256, 512)
    ...
    for d, c, w, se in zip(d, c, wf, se_blocks):
        ...
        for i in range(d):
            ...
            if i >= d - se:
                ak["attn_layer"] = "se"
            scfg += [
                ByoBlockCfg(type="one", d=1, c=prev_c, gs=1, block_kwargs=bk, **ak)
            ]  # depthwise block
            scfg += [
                ByoBlockCfg(
                    type="one",
                    ...
                    c=out_c,
                    gs=0,
                    block_kwargs=dict(kernel_size=1, **bk),
                    **ak,
                )
            ]  # pointwise block
            prev_c = out_c
...
class BasicBlock(nn.Module):
    ...
    def init_weights(self, zero_init_last: bool = False):
        if (
            ...
            and getattr(self.conv2_kxk.bn, "weight", None) is not None
        ):
...
class BottleneckBlock(nn.Module):
    ...
    def init_weights(self, zero_init_last: bool = False):
        if (
            zero_init_last
            and self.shortcut is not None
            and getattr(self.conv3_1x1.bn, "weight", None) is not None
        ):
...
class EdgeBlock(nn.Module):
    ...
    def init_weights(self, zero_init_last: bool = False):
        if (
            zero_init_last
            and self.shortcut is not None
            and getattr(self.conv2_1x1.bn, "weight", None) is not None
        ):
...
class RepVggBlock(nn.Module):
    ...
    def __init__(
        ...
        if inference_mode:
            ...
            self.identity = (
                layers.norm_act(out_chs, apply_act=False) if use_ident else None
            )
...
class Stem(nn.Sequential):
    def __init__(
        ...
        if pool:
            ...
            last_feat_idx = i
            self.feature_info.append(
                dict(num_chs=prev_chs, reduction=curr_stride, module=prev_feat, stage=0)
            )
            if pool == "max2":
...
def create_byob_stem(
    ...
    if "quad" in stem_type:
        ...
        if isinstance(out_chs, (tuple, list)):
            ...
            if pool_type:
                stem = Stem(
                    in_chs, out_chs, 3, num_rep=1, pool=pool_type, layers=layers
                )
            else:
...
def get_layer_fns(cfg: ByoModelCfg, allow_aa: bool = True):
    ...
    if cfg.aa_layer and allow_aa:
        conv_norm_act = partial(
            ConvNormAct, norm_layer=cfg.norm_layer, act_layer=act, aa_layer=cfg.aa_layer
        )
    else:
...
def _convert_openai_clip(
    ...
    def _down_sub(m):
        ...
        return f"stages.{stage_idx}.{layer_idx}.shortcut." + (
            "conv.conv" if layer_id == 0 else "conv.bn"
        )
    ...
    for k, v in state_dict.items():
        ...
        k = re.sub(
            rf"{prefix}layer([0-9])\.([0-9]+)\.downsample\.([0-9])", _down_sub, k
        )
