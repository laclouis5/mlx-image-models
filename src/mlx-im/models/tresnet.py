...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = ConvNormAct(
            planes, planes, kernel_size=3, stride=1, apply_act=False
        )
...
class TResNet(nn.Module):
    def __init__(
        ...
        conv1 = ConvNormAct(
            in_chans * 16, self.planes, stride=1, kernel_size=3, act_layer=act_layer
        )
        ...
        self.feature_info = [
            ...
            dict(
                num_chs=self.planes * (Bottleneck.expansion if v2 else 1),
                reduction=4,
                module="body.layer1",
            ),
            dict(
                num_chs=self.planes * 2 * (Bottleneck.expansion if v2 else 1),
                reduction=8,
                module="body.layer2",
            ),
            dict(
...
def checkpoint_filter_fn(state_dict, model):
    ...
    for k, v in state_dict.items():
        ...
        k = re.sub(
            r"downsample\.(\d+)\.0", lambda x: f"downsample.{int(x.group(1))}.conv", k
        )
        k = re.sub(
            r"downsample\.(\d+)\.1", lambda x: f"downsample.{int(x.group(1))}.bn", k
        )
...
default_cfgs = generate_default_cfgs(
    {
        ...
        "tresnet_m.miil_in1k_448": _cfg(
            input_size=(3, 448, 448), pool_size=(14, 14), hf_hub_id="timm/"
        ),
...
def tresnet_m(pretrained=False, **kwargs) -> TResNet:
    ...
    return _create_tresnet(
        "tresnet_m", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def tresnet_l(pretrained=False, **kwargs) -> TResNet:
    ...
    return _create_tresnet(
        "tresnet_l", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def tresnet_xl(pretrained=False, **kwargs) -> TResNet:
    ...
    return _create_tresnet(
        "tresnet_xl", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def tresnet_v2_l(pretrained=False, **kwargs) -> TResNet:
    ...
    return _create_tresnet(
        "tresnet_v2_l", pretrained=pretrained, **dict(model_args, **kwargs)
    )
