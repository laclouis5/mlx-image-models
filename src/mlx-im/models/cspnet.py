...
class CspStagesCfg:
    ...
    bottle_ratio: Union[float, Tuple[float, ...]] = (
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn2 = (
            attn_layer(mid_chs, act_layer=act_layer) if attn_first else nn.Identity()
        )
        self.conv3 = ConvNormAct(
            mid_chs, out_chs, kernel_size=1, apply_act=False, **ckwargs
        )
        self.attn3 = (
            attn_layer(out_chs, act_layer=act_layer) if attn_last else nn.Identity()
        )
...
model_cfgs = dict(
    ...
    cs3sedarknet_x=_cs3_cfg(
        attn_layer="se", width_multiplier=1.25, depth_multiplier=1.33
    ),
