...
class PositionalEncodingFourier(nn.Module):
    ...
    def forward(self, shape: Tuple[int, int, int]):
        ...
        dim_t = torch.arange(self.hidden_dim, dtype=torch.int64, device=device).to(
            torch.float32
        )
...
def edgenext_xx_small(pretrained=False, **kwargs) -> EdgeNeXt:
    ...
    return _create_edgenext(
        "edgenext_xx_small", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def edgenext_x_small(pretrained=False, **kwargs) -> EdgeNeXt:
    ...
    return _create_edgenext(
        "edgenext_x_small", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def edgenext_small(pretrained=False, **kwargs) -> EdgeNeXt:
    ...
    return _create_edgenext(
        "edgenext_small", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def edgenext_base(pretrained=False, **kwargs) -> EdgeNeXt:
    ...
    return _create_edgenext(
        "edgenext_base", pretrained=pretrained, **dict(model_args, **kwargs)
    )
