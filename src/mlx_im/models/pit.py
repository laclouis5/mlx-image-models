...
class ConvEmbedding(nn.Module):
    def __init__(
        ...
        self.height = math.floor(
            (self.img_size[0] + 2 * padding - self.patch_size[0]) / stride + 1
        )
        self.width = math.floor(
            (self.img_size[1] + 2 * padding - self.patch_size[1]) / stride + 1
        )
...
class PoolingVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = ConvEmbedding(
            in_chans, embed_dim, img_size, patch_size, stride
        )
        self.pos_embed = nn.Parameter(
            torch.randn(1, embed_dim, self.patch_embed.height, self.patch_embed.width)
        )
        ...
        for i in range(len(depth)):
            ...
            self.feature_info += [
                dict(
                    num_chs=prev_dim,
                    reduction=(stride - 1) * 2**i,
                    module=f"transformers.{i}",
                )
            ]
...
default_cfgs = generate_default_cfgs(
    {
        ...
        "pit_ti_distilled_224.in1k": _cfg(
            hf_hub_id="timm/", classifier=("head", "head_dist")
        ),
