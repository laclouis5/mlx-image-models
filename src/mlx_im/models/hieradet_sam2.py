...
def window_unpartition(
    windows: torch.Tensor, window_size: Tuple[int, int], hw: Tuple[int, int]
):
    """
...
def _calc_pad(
    H: int, W: int, window_size: Tuple[int, int]
...
class MultiScaleBlock(nn.Module):
    def __init__(
        ...
        self.ls1 = (
            LayerScale(dim_out, init_values)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.q_stride is not None:
            ...
            window_size = (
                self.window_size[0] // self.q_stride[0],
                self.window_size[1] // self.q_stride[1],
            )
...
class HieraDet(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(self.head, ClNormMlpClassifierHead) and isinstance(
            self.head.fc, nn.Linear
        ):
    ...
    def forward_intermediates(
        ...
        assert output_fmt in (
            "NCHW",
            "NHWC",
        ), "Output format must be one of NCHW, NHWC."
        if coarse:
...
def sam2_hiera_tiny(pretrained=False, **kwargs):
    ...
    return _create_hiera_det(
        "sam2_hiera_tiny", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def sam2_hiera_small(pretrained=False, **kwargs):
    ...
    return _create_hiera_det(
        "sam2_hiera_small", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def sam2_hiera_base_plus(pretrained=False, **kwargs):
    ...
    return _create_hiera_det(
        "sam2_hiera_base_plus", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def sam2_hiera_large(pretrained=False, **kwargs):
    ...
    return _create_hiera_det(
        "sam2_hiera_large", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def hieradet_small(pretrained=False, **kwargs):
    ...
    return _create_hiera_det(
        "hieradet_small", pretrained=pretrained, **dict(model_args, **kwargs)
    )
