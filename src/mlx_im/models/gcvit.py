...
class WindowAttentionGlobal(nn.Module):
    ...
    def forward(self, x, q_global: Optional[torch.Tensor] = None):
        ...
        if self.use_global and q_global is not None:
            _assert(
                x.shape[-1] == q_global.shape[-1],
                "x and q_global seq lengths should be equal",
            )

            kv = self.qkv(x)
            kv = kv.reshape(B, N, 2, self.num_heads, self.head_dim).permute(
                2, 0, 3, 1, 4
            )
            k, v = kv.unbind(0)
        ...
        )  # NOTE contiguous() fixes an odd jit bug in PyTorch 2.0
...
class GlobalContextVitBlock(nn.Module):
    def __init__(
        ...
        self.num_windows = int(
            (feat_size[0] // window_size[0]) * (feat_size[1] // window_size[1])
        )
        ...
        for i in range(num_stages):
            ...
            stages.append(
                GlobalContextVitStage(
                    ...
                    feat_size=(
                        feat_size[0] // stage_scale,
                        feat_size[1] // stage_scale,
                    ),
