...
class EfficientNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv_stem = create_conv2d(
            in_chans, stem_size, stem_kernel_size, stride=2, padding=pad_type
        )
    ...
    def forward_intermediates(
        ...
        if extra_blocks:
            take_indices, max_index = feature_take_indices(
                len(self.blocks) + 1, indices
            )
        else:
            take_indices, max_index = feature_take_indices(
                len(self.stage_ends), indices
            )
...
class EfficientNetFeatures(nn.Module):
    ...
    def __init__(
        ...
        self._stage_out_idx = {
            f["stage"]: f["index"] for f in self.feature_info.get_dicts()
        }
...
def _create_effnet(variant, pretrained=False, **kwargs):
    ...
    if features_mode == "cls":
        model.pretrained_cfg = model.default_cfg = pretrained_cfg_for_features(
...
def _gen_mobilenet_v1(
    ...
    **kwargs,
    ...
    round_chs_fn = partial(round_channels, multiplier=channel_multiplier)
    head_features = (
        (1024 if fix_stem_head else max(1024, round_chs_fn(1024))) if head_conv else 0
    )
    model_kwargs = dict(
    ...
    if "edgetpu_v2" in variant:
        ...
        def _arch_def(chs: List[int], group_size: int):
            return [
                ...
                ],  # NOTE with expansion==1, official impl block ends just 1x1 pwl
                # stage 1, 112x112 in
                [
                    f"er_r1_k3_s2_e8_c{chs[1]}",
                    f"er_r1_k3_s1_e4_gs{group_size}_c{chs[1]}",
                ],
                # stage 2, 56x56 in
                [
...
default_cfgs = generate_default_cfgs(
    {
        ...
        "efficientnet_cc_b1_8e.untrained": _cfg(
...
def mobilenetv1_100h(pretrained=False, **kwargs) -> EfficientNet:
    """MobileNet V1"""
    model = _gen_mobilenet_v1(
        "mobilenetv1_100h", 1.0, head_conv=True, pretrained=pretrained, **kwargs
    )
    return model
...
def mobilenet_edgetpu_100(pretrained=False, **kwargs) -> EfficientNet:
    ...
    model = _gen_mobilenet_edgetpu(
        "mobilenet_edgetpu_100", pretrained=pretrained, **kwargs
    )
    return model
...
def mobilenet_edgetpu_v2_xs(pretrained=False, **kwargs) -> EfficientNet:
    ...
    model = _gen_mobilenet_edgetpu(
        "mobilenet_edgetpu_v2_xs", pretrained=pretrained, **kwargs
    )
    return model
...
def mobilenet_edgetpu_v2_s(pretrained=False, **kwargs) -> EfficientNet:
    ...
    model = _gen_mobilenet_edgetpu(
        "mobilenet_edgetpu_v2_s", pretrained=pretrained, **kwargs
    )
    return model
...
def mobilenet_edgetpu_v2_m(pretrained=False, **kwargs) -> EfficientNet:
    ...
    model = _gen_mobilenet_edgetpu(
        "mobilenet_edgetpu_v2_m", pretrained=pretrained, **kwargs
    )
    return model
...
def mobilenet_edgetpu_v2_l(pretrained=False, **kwargs) -> EfficientNet:
    ...
    model = _gen_mobilenet_edgetpu(
        "mobilenet_edgetpu_v2_l", pretrained=pretrained, **kwargs
    )
    return model
...
def test_efficientnet_evos(pretrained=False, **kwargs) -> EfficientNet:
    model = _gen_test_efficientnet(
        ...
        pretrained=pretrained,
        norm_layer=partial(EvoNorm2dS0, group_size=8),
        **kwargs,
    )
    return model
