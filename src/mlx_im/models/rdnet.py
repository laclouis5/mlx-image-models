...
class DenseStage(nn.Sequential):
    def __init__(
        self, num_block, num_input_features, drop_path_rates, growth_rate, **kwargs
    ):
        super().__init__()
...
class RDNet(nn.Module):
    def __init__(
        ...
        is_downsample_block: Union[List[bool], Tuple[bool]] = (
            None,
            True,
            True,
            False,
        ...
        if stem_type == "patch":
            ...
            mid_chs = (
                make_divisible(num_init_features // 2)
        ...
        dp_rates = [
            x.tolist()
            for x in torch.linspace(0, drop_path_rate, sum(num_blocks_list)).split(
                num_blocks_list
            )
        ]
        ...
        for i in range(self.num_stages):
            ...
            if i != 0:
                compressed_num_features = (
                    int(num_features * transition_compression_ratio / 8) * 8
                )
                k_size = stride = 1
    ...
    def forward_intermediates(
        ...
        take_indices, max_index = feature_take_indices(
            len(self.dense_stages) + 1, indices
        )
    ...
    def prune_intermediate_layers(
        ...
        self.dense_stages = self.dense_stages[
            :max_index
        ]  # truncate blocks w/ stem as idx 0
        if prune_norm:
            self.norm_pre = nn.Identity()
        if prune_head:
...
def rdnet_tiny(pretrained=False, **kwargs):
    ...
    model = _create_rdnet(
        "rdnet_tiny", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def rdnet_small(pretrained=False, **kwargs):
    ...
    model = _create_rdnet(
        "rdnet_small", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def rdnet_base(pretrained=False, **kwargs):
    ...
    model = _create_rdnet(
        "rdnet_base", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def rdnet_large(pretrained=False, **kwargs):
    ...
    model = _create_rdnet(
        "rdnet_large", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
