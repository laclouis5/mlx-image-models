...
class HybridEmbed(nn.Module):
    ...
    def __init__(
        ...
        if not dynamic_img_pad:
            assert (
                self.feature_size[0] % self.patch_size[0] == 0
                and self.feature_size[1] % self.patch_size[1] == 0
            )
        ...
        if feature_size is None:
            with torch.no_grad():
                ...
                if training:
                    self.backbone.eval()
                o = self.backbone(
                    torch.zeros(1, self.in_chans, img_size[0], img_size[1])
                )
    ...
    def set_input_size(
        ...
        if new_patch_size is not None and new_patch_size != self.patch_size:
            assert isinstance(
                self.proj, nn.Conv2d
            ), "HybridEmbed must have a projection layer to change patch size."
            with torch.no_grad():
                new_proj = nn.Conv2d(
    ...
    def dynamic_feat_size(self, img_size: Tuple[int, int]) -> Tuple[int, int]:
        ...
        feat_size = (
            img_size[0] // self.feature_ratio[0],
            img_size[1] // self.feature_ratio[1],
        )
        if self.dynamic_img_pad:
            return math.ceil(feat_size[0] / self.patch_size[0]), math.ceil(
                feat_size[1] / self.patch_size[1]
            )
        else:
            return feat_size[0] // self.patch_size[0], feat_size[1] // self.patch_size[
                1
            ]
