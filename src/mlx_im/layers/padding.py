...
def get_same_padding(x: int | mx.array, kernel_size: int, stride: int, dilation: int):
    if isinstance(x, mx.array):
        return mx.clip(
            (mx.ceil(x / stride) - 1) * stride + (kernel_size - 1) * dilation + 1 - x,
            a_min=0,
        )
    else:
        return max(
...
def get_padding_value(
    padding: str | tuple[int], kernel_size: int, **kwargs
) -> Tuple[Tuple, bool]:
    dynamic = False
